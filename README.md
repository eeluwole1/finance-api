...
- **Payments** — Premium payments with full status tracking
...
| Testing        | JUnit 5 · Mockito · EqualsVerifier 3.15 |
...
```
src/
├── main/java/com/eeluwole/finance_api/
│   ├── auth/
│   │   ├── User.java
│   │   ├── UserRepository.java
...
└── test/java/com/eeluwole/finance_api/
    ├── account/   (AccountTest, AccountServiceTest)
    ├── advanced/  (AdvancedTest, AdvancedServiceTest)
    ├── auth/      (UserTest, AuthServiceTest)
    ├── beneficiary/ (BeneficiaryTest, BeneficiaryServiceTest)
    ├── claims/    (ClaimTest, ClaimServiceTest)
    ├── client/    (ClientTest, ClientServiceTest)
    ├── payment/   (PaymentTest, PaymentServiceTest)
    ├── policy/    (PolicyTest, PolicyServiceTest)
    ├── rewards/   (RewardTest, RewardServiceTest)
    └── transaction/ (TransactionTest, TransactionServiceTest)
```
...
The project has **108 unit tests** across all 10 modules — zero integration tests required, no database connection needed to run the test suite.
...
| Module      | Entity Test | Service Tests | Test Count |
|-------------|-------------|---------------|------------|
| account     | AccountTest | AccountServiceTest | 19 |
| advanced    | AdvancedTest | AdvancedServiceTest | 10 |
| auth        | UserTest | AuthServiceTest | 5 |
| beneficiary | BeneficiaryTest | BeneficiaryServiceTest | 10 |
| claims      | ClaimTest | ClaimServiceTest | 10 |
| client      | ClientTest | ClientServiceTest | 10 |
| payment     | PaymentTest | PaymentServiceTest | 10 |
| policy      | PolicyTest | PolicyServiceTest | 11 |
| rewards     | RewardTest | RewardServiceTest | 11 |
| transaction | TransactionTest | TransactionServiceTest | 11 |
| **Total**   | | | **108** |

### Test patterns used

**Entity tests** — verify `equals()` and `hashCode()` correctness using EqualsVerifier:
```java
@Test
...
**Service tests** — full Mockito unit tests using `@Mock` repositories and `@InjectMocks` service:
```java
...
> Savings, chequing, and investment accounts held by a client. Supports real-time deposits and withdrawals with balance tracking (max balance: $1,000,000).
...
| GET    | `/policies/client/{clientId}`       | Get policies for a client|
| GET    | `/policies/status/{status}`         | Filter by status         |
...
> Records of premium payments made by clients to keep their policies active.

| Method | Endpoint                            | Description               |
|--------|-------------------------------------|---------------------------|
| GET    | `/payments`                         | Get all payments          |
| GET    | `/payments/{id}`                    | Get a single payment      |
| GET    | `/payments/client/{clientId}`       | Get payments by a client  |
| GET    | `/payments/policy/{policyId}`       | Get payments on a policy  |
| GET    | `/payments/status/{status}`         | Filter by status          |
| POST   | `/payments`                         | Record a new payment      |
| PATCH  | `/payments/{id}/status?status=`     | Update payment status     |
| DELETE | `/payments/{id}`                    | Delete a payment          |

**Payment methods:** `CREDIT_CARD` · `BANK_TRANSFER` · `CHEQUE` · `DIRECT_DEBIT`
...
**Claim types:** `MEDICAL` · `AUTO` · `HOME` · `LIFE`
...
| GET    | `/loans/client/{clientId}`      | Get loans for a client|
| GET    | `/loans/status/{status}`        | Filter by status     |
| POST   | `/loans`                        | Create a policy loan |
...
| GET    | `/transactions/client/{clientId}`     | Get transactions for a client  |
...
| GET    | `/rewards/client/{clientId}/points`     | Get total active points for client |
...
**Reward types:** `EARNED` · `REDEEMED`

**Reward statuses:** `ACTIVE` · `EXPIRED` · `USED`
...
6. PATCH /api/v1/payments/1/status    → status=COMPLETED (payment cleared)
...
2. PATCH /api/v1/payments/2/status    → status=FAILED (insufficient funds)
3. PATCH /api/v1/clients/1/status     → status=SUSPENDED
4. POST /api/v1/payments              → Client retries payment
5. PATCH /api/v1/payments/3/status    → status=COMPLETED (success)
